...
internal static class LocationCoordinateLookup
{
	...
	public static (double Latitude, double Longitude) Resolve(LocationProfile location)
	{
		string key = $"{location.CountryCode}|{location.Region}";
		if (!string.IsNullOrWhiteSpace(location.Region) && Regions.TryGetValue(key, out (double Latitude, double Longitude) region))
		{
			return region;
		}

		return CountryCenters.TryGetValue(location.CountryCode, out (double Latitude, double Longitude) country)
			? country
			: (20.0, 0.0);
	}
}
